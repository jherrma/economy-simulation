// Zig, hand-rolled inlined insertion sort — the idiom used in the results table.
// Mirrors bench_c2.c exactly, so the comparison is sort-for-sort rather than
// std.mem.sort against a hand-written loop. See bench.zig for the library-sort variant.
...
fn isort(a: []Cand) void {
    var i: usize = 1;
    while (i < a.len) : (i += 1) {
        const k = a[i];
        var j: isize = @as(isize, @intCast(i)) - 1;
        while (j >= 0 and a[@intCast(j)].score < k.score) : (j -= 1) {
